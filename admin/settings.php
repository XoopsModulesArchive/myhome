...
// Options ///////////////////////////////////////////////////////////////////////////////////////////
   ...
   $xoops_cp_header   = 1;        // Use or not the xoops_cp_header function. Depends on the modules
...
<ul>
  ...
  <li>Edit your language/english/modinfo.php file and enter the new language define as following
    ...
    ** If you don't create your custom define, a default title will appear with a red <font color='red'>*</font>.
    ...
    Exemple :<ol>
            <li>_MI_MYHOME_BANNER</li>     <li>_MI_MYHOME_TEXTE_INDEX</li>        <li>_MI_MYHOME_VALUE1</li>      <li>_MI_MYHOME_VALUE2</li></ol>
  </li>
  <li>Customise the script and tab look.
                                                                                                               
   Edit file for more options.</li>
   </ul>
   <font color='red'>
...
// Display editor
  ...
  case ( 'save' ):
                      ...
                      if (!$GLOBALS['xoopsSecurity']->check()) {
                        redirect_header("?admin=settings", 3, implode('<br />', $GLOBALS['xoopsSecurity']->getErrors()));
                    }
             ...
             //             header('Location: ?admin=settings&sub='.$sub);
                        redirect_header('?admin=settings&sub='.$sub,0,'');
  break;
}
...
function settings_display( $sub='', $num='' ) {
        ...
        $count = count($config);
        if ($count < 1) {
            redirect_header('?admin=settings', 1);
        }
        include_once XOOPS_ROOT_PATH.'/class/xoopsformloader.php';
        $module_handler =& xoops_gethandler('module');
        $module =& $module_handler->get($mod);
        $cat  = '<font style="color:Yellow;">'.constant( strtoupper('_MI_'. $xoopsModule->dirname() . '_' . $sub)).'</font>';
        $form = new XoopsThemeForm(_PREFERENCES . ' ' . $module->getVar('name') . ' : '.$cat, 'pref_form', '?admin=settings&sub='.$sub, 'post', true);
        ...
        for ($i = 0; $i < $count; $i++) {
            $cat    = explode('_',$config[$i]->getVar('conf_name'));
            $color  = $num?'Red':'Grey';
            $title  = isset($cat[0])?'<sup style="color:'.$color.';">['.strtoupper(constant( strtoupper('_MI_'. $xoopsModule->dirname() . '_' . $cat[0]))).']</sup> ':'<sup>[COMMON]</sup>';
            ...
            case 'color':
                $myts =& MyTextSanitizer::getInstance();
                $ele = new XoopsFormColorPicker($title, $config[$i]->getVar('conf_name'), $myts->htmlspecialchars($config[$i]->getConfValueForOutput()), $myts->htmlspecialchars($config[$i]->getConfValueForOutput()));
                break;
            case 'hidden':
                $myts =& MyTextSanitizer::getInstance();
...
foreach( $xoopsModuleConfig as $name=>$value ) {
         ...
         if( !ereg($cat[0],$cat_liste) && isset($cat[1]) ) {
           ...
           $sub_menu[$i]['link']    = '?admin=settings&sub='.$cat[0] . $description;
           $sub_menu[$i]['cat']     = $cat[0];
           $sub_menu[$i]['color']   = isset($colors[$i])?$colors[$i]:$default_color;
           $cat_liste .='|'.$cat[0];
         }
  }
    ...
    break;
     ...
     case ('select'):
          ...
          foreach( $sub_menu as $submenu ) {
                   ...
                   $info     = $selected=='selected'?@constant( strtoupper('_MI_'. $xoopsModule->dirname() . '_' . $submenu['cat'] . 'DSC') ):$info;
                   $nbsp = $i++?'&nbsp;&deg; ':'';
