...
$file = myhome_tpl_rename ( $xoopsModuleConfig['index_mode'] );
...
function myhome_select_tpl ( $def_menu, $tpl='' ) {
  ...
  foreach( $css_array as $i=>$file_data ) {
                             $file = myhome_tpl_rename ( $file_data );
                             $css_file .= $file['file_name']!='index'?' | ' . $file['file_name'] :'';
                             }

  $list .= '<option value=""></option>';
  $xoopsModuleConfig['item_selectmode'] = XoopsLists :: getHtmlListAsArray( XOOPS_ROOT_PATH . '/modules/myhome/templates/' );
...
function myhome_write_file( $file, $data ) {

       	$handle = fopen($file, 'w+');
		if ($handle) {
			if ( fwrite($handle, $data) ) {
                        include (XOOPS_ROOT_PATH.'/class/xoopsformloader.php');
                          $file_array  = XoopsLists :: getFileListAsArray( XOOPS_ROOT_PATH . '/uploads/myhome/cache' );
                          $file_reg    = explode( '0', basename ($file) );
                          foreach( $file_array as $files ) {
                            if( ereg( $file_reg[0], $files ) && !ereg($file_reg[0].'0', $files) ) { 
                                unlink( XOOPS_ROOT_PATH . '/uploads/myhome/cache/'.$files );   // Delete temp files
                                }
                          }
                          return true; }
...
function myhome_read_file( $file ) {
...
function myhome_unlink_cache_files( $tpl ) {
  include XOOPS_ROOT_PATH."/class/xoopslists.php";              
...
// Update a data
  ...
  $i=0;
        // CSS
        $css_text  = '<p />' . _MD_MYHOME_CSS . ' : ';
        if( $css ) {
        $css_text .= myhome_write_file( $css_file, $css )?_MD_MYHOME_UPDATED:_MD_MYHOME_NOTUPDATED;
        $i++;
        } elseif( file_exists($css_file) && !$css  ) {
        $css_text .= unlink( $css_file )?_MD_MYHOME_DELETED:_MD_MYHOME_NOTDELETED;
        $i++;
        } else { $css_text = ''; }

        // Script
        $script_text = '<p />' . _MD_MYHOME_SCRIPT . ' : ';
        if( $script ) {
        $script_text .= myhome_write_file( $script_file, $script )?_MD_MYHOME_UPDATED:_MD_MYHOME_NOTUPDATED;
        $i++;
        } elseif( file_exists($script_file) && !$script  ) {
        $script_text .= unlink( $script_file )?_MD_MYHOME_DELETED:_MD_MYHOME_NOTDELETED;
        $i++;
        } else { $script_text = ''; }
        
        
        // Template
        $template_text = '<p />' . _MD_MYHOME_TEMPLATE . ' : ';
        ...
        $tpl_source      = $myts->AddSlashes($tpl_source);
       	$sql = "UPDATE ".$xoopsDB->prefix('tplsource')."
	        ...
	        WHERE tpl_id   = '$tpl_id'";
// 	$xoopsDB->queryF($sql)?_MD_MYHOME_UPDATED:_MD_MYHOME_NOTUPDATED;
        if( $xoopsDB->queryF($sql) ) { myhome_unlink_cache_files( $tpl );
                                                     $template_text .= _MD_MYHOME_UPDATED; }
 	$i++;
 	} else { $template_text = ''; }
  ...
  default:
  	include XOOPS_ROOT_PATH."/class/xoopsformloader.php";
	$form = new XoopsThemeForm(_MD_MYHOME_CSS . myhome_select_tpl( $def_menu, $tpl ), "newform", $def_menu."&op=post");
