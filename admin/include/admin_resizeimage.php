...
function myhome_thumb_resize ( $image_list, $xmax='200', $ymax='150', $thumb='thumb/', $image='', $fond_couleur='#FFFFFF', $force=0 ) {
/*
...
if( $force==1 ) {
  myhome_delete_files( $thumb );
   } elseif( $force==2 ) {
     Global $extensions;
  myhome_create_dir( $image );
  myhome_move_files( $image, $image.'orig/' );
   }
          ...
          // si le fichier n'est pas une image :
              if(list($largeur,$hauteur,$extention) = @getimagesize(htmlentities($img))) {
                  ...
                  // creation de la miniature :
                      ...
                      list($fond_rouge,$fond_vert,$fond_bleu) = explode('-',myhome_html2rgb($fond_couleur));
...
function myhome_html2rgb($color)
{
    if ($color[0] == '#') {
...
function myhome_delete_files( $dir='', $ext='html' ) {
...
function myhome_move_files( $dir='', $newdir='', $extensions='jpg' ) {
// Global $extensions;
// Get image list
        //Using the opendir function
        $dir_handle = @opendir($dir);
...
function myhome_create_dir( $directory='' )
{
...
function myhome_mkdir($target)
{
	...
	if (myhome_mkdir(substr($target,0,strrpos($target,'/')))) {
...
function myhome_chmod($target, $mode = 0777)
{
