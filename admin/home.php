...
// Menus
         $sql = "SELECT catid, title, description
                 FROM " . $xoopsDB->prefix( 'myhome_menu' ) . "
                 ORDER BY title ASC";
	...
	{
                  $ret_menu .= '<li>';
                  $ret_menu .= ' [<a href="index.php?admin=menu&catid='.$l_catid.'&op=edit" title="'.strip_tags($l_description).'">'._MD_MYHOME_MENU.'</a>]';
                  $ret_menu .= ' [<a href="index.php?admin=link&catid='.$l_catid.'&op=edit" title="'.strip_tags($l_description).'">'._MD_MYHOME_LINK.'</a>] ';
                  $ret_menu .= $l_title . '</li>';
        }
                  $ret_menu .= '</ul>';
...
// sql
         $sql = "SELECT id, title, qtable
                 FROM " . $xoopsDB->prefix( 'myhome_query' ) . "
                 ORDER BY title ASC";
         $result = $xoopsDB->queryF( $sql);
                  $ret_query = '<ul>';
